...
type AssetRepository interface {
	...
	ListAssetsByStartup(ctx context.Context, startupID int64, limit, offset int) ([]Asset, int64, error)
}
...
func (r *postgresAssetRepository) CreateAsset(ctx context.Context, input Asset) (Asset, error) {
	query := `INSERT INTO assets (startup_id, title, description, asset_type, image_url, price, is_negotiable, is_sold, is_active, created_at)
              VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, NOW())
              RETURNING id, startup_id, title, description, asset_type, image_url, price, is_negotiable, is_sold, is_active, created_at`

	row := r.pool.QueryRow(ctx, query, input.StartupID, input.Title, input.Description, input.AssetType, input.ImageURL, input.Price, input.IsNegotiable, input.IsSold, input.IsActive)

	var created Asset
	if err := row.Scan(&created.ID, &created.StartupID, &created.Title, &created.Description, &created.AssetType, &created.ImageURL, &created.Price, &created.IsNegotiable, &created.IsSold, &created.IsActive, &created.CreatedAt); err != nil {
		return Asset{}, err
	}
...
func (r *postgresAssetRepository) UpdateAsset(ctx context.Context, input Asset) (Asset, error) {
	...
	var updated Asset
	if err := row.Scan(&updated.ID, &updated.StartupID, &updated.Title, &updated.Description, &updated.AssetType, &updated.ImageURL, &updated.Price, &updated.IsNegotiable, &updated.IsSold, &updated.IsActive, &updated.CreatedAt); err != nil {
...
func (r *postgresAssetRepository) GetAssetByID(ctx context.Context, id int64) (Asset, error) {
	query := `SELECT id, startup_id, title, description, asset_type, image_url, price, is_negotiable, is_sold, is_active, created_at
              FROM assets
	...
	var a Asset
	if err := row.Scan(&a.ID, &a.StartupID, &a.Title, &a.Description, &a.AssetType, &a.ImageURL, &a.Price, &a.IsNegotiable, &a.IsSold, &a.IsActive, &a.CreatedAt); err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return Asset{}, ErrAssetNotFound
		}
		return Asset{}, err
...
func (r *postgresAssetRepository) ListAssets(ctx context.Context, filters AssetFilters, limit, offset int) ([]Asset, int64, error) {
	...
	query := fmt.Sprintf(`SELECT id, startup_id, title, description, asset_type, image_url, price, is_negotiable, is_sold, is_active, created_at
              FROM assets
              %s
              ORDER BY id
	...
	for rows.Next() {
		var a Asset
		if err := rows.Scan(&a.ID, &a.StartupID, &a.Title, &a.Description, &a.AssetType, &a.ImageURL, &a.Price, &a.IsNegotiable, &a.IsSold, &a.IsActive, &a.CreatedAt); err != nil {
			return nil, 0, err
		}
...
func (r *postgresAssetRepository) ListAssetsByStartup(ctx context.Context, startupID int64, limit, offset int) ([]Asset, int64, error) {
	query := `SELECT id, startup_id, title, description, asset_type, image_url, price, is_negotiable, is_sold, is_active, created_at
              FROM assets
              WHERE startup_id = $1 AND is_active = true AND is_deleted = false
              ORDER BY id
              LIMIT $2 OFFSET $3`

	rows, err := r.pool.Query(ctx, query, startupID, limit, offset)
	if err != nil {
		return nil, 0, err
	}
	defer rows.Close()
	...
	var total int64
	countRow := r.pool.QueryRow(ctx, "SELECT COUNT(*) FROM assets WHERE startup_id = $1 AND is_active = true AND is_deleted = false", startupID)
	if err := countRow.Scan(&total); err != nil {
		return nil, 0, err
	}
